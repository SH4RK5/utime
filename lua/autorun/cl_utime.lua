...
function think()
	...
	gpanel:SetPos((ScrW() - gpanel:GetWide()) * utime_pos_x:GetFloat() / 100,
	...
	local insideTextColor = Color(utime_insidetext_r:GetInt(), utime_insidetext_g:GetInt(), utime_insidetext_b:GetInt(),
		255)
...
function PANEL:Paint(w, h)
	...
	local outerColor = Color(utime_outsidecolor_r:GetInt(), utime_outsidecolor_g:GetInt(), utime_outsidecolor_b:GetInt(),
		200)
...
function PANEL:Think()
	...
	if self.Size ~= self.TargetSize then
		self.Size = math.Approach(self.Size, self.TargetSize,
			(math.abs(self.Size - self.TargetSize) + 1) * 8 * FrameTime())
		self:PerformLayout()
	end
